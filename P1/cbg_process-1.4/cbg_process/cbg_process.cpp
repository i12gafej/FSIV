...
main (int argc, char* const* argv)
{
  ...
  try {    
      ...
      //TODO
      //entrada
      input = cv::imread(input_name);
      if (input.empty()) {
          std::cerr << "Error: could not open the input image file '" << input_name << "'." << std::endl;
          return EXIT_FAILURE;
      }

      double contrast = parser.get<double>("c");
      double brightness = parser.get<double>("b");
      double gamma = parser.get<double>("g");
      bool only_luma = parser.has("l");
      //salida

      output = cbg_process(input, output, contrast, brightness, gamma, only_luma);
