...
void on_mouse (int event, int x, int y, int flags, void * user_data_)
{
    ...
    if (event == cv::EVENT_LBUTTONDOWN)
    {
        ...
        cv::Point point;
        point.x = x, point.y = y;
        user_data->output = fsiv_color_rescaling(user_data->input, user_data->input.at<cv::Vec3b>(point), cv::Scalar(255,255,255));
...
main (int argc, char* const* argv)
{
    ...
    try {
        ...
        if(input.empty()){
            std::cerr << "Error: could not open the input image " << input_n << std::endl;
            return EXIT_FAILURE;
        }
