                           ...
                           float gamma)
{
    ...
    //Set it as a classifier (setIsClassifier) <-- es isClassifier
    svm->isClassifier();
                              ...
                              float E)
{
    ...
    // @see opencv docs.
    cv::TermCriteria tcrit = cv::TermCriteria(cv::TermCriteria::MAX_ITER + cv::TermCriteria::EPS, T, E);
    // Set hyperparameters: V, T, E.
    rtrees->setActiveVarCount(V);
    rtrees->setTermCriteria(tcrit);
    //
...
fsiv_load_knn_classifier_model(const std::string &model_fname)
{
    ...
    clsf = cv::ml::StatModel::load<cv::ml::KNearest>(model_fname);
    //
...
fsiv_load_svm_classifier_model(const std::string &model_fname)
{
    ...
    // Hint: use the generic interface cv::Algorithm::load< classifier_type >
    clsf = cv::ml::StatModel::load<cv::ml::SVM>(model_fname);
    //
...
fsiv_load_rtrees_classifier_model(const std::string &model_fname)
{
    ...
    // Hint: use the generic interface cv::Algorithm::load< classifier_type >
    clsf = cv::ml::StatModel::load<cv::ml::RTrees>(model_fname);
    //
