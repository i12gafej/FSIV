...
main (int argc, char* const* argv)
{
    ...
    try
    {
        ...
        cv::FileStorage fileS(intrinsics_file, cv::FileStorage::READ);

        cv::Size tamano;
        float error;
        cv::Mat matriz;
        cv::Mat coeffs, rvec, tvec;
        fsiv_load_calibration_parameters(fileS, tamano, error, matriz, coeffs, rvec, tvec);
        fileS.release();
        //
        ...
        p3D = fsiv_generate_3d_calibration_points(board_size, size);
        //
        ...
        {            
            //TODO
             if(cap.read(input_frame) && !input_frame.empty()){
                bool was_found = fsiv_find_chessboard_corners(input_frame, board_size, p2D);
                if(was_found){
                    fsiv_compute_camera_pose(p3D, p2D, matriz, coeffs, rvec, tvec);
                    if(!projected_image.empty())
                        fsiv_project_image(projected_image, input_frame, board_size, p2D);
                    else if(projected_video.isOpened()){
                        cv::Mat input_video;
                        if(projected_video.read(input_video) && !input_video.empty())
                            fsiv_project_image(input_video, input_frame, board_size, p2D);
                    }
                    else if(draw_axis)
                        fsiv_draw_axes(input_frame, matriz, coeffs, rvec, tvec, size);
                    else
                        fsiv_draw_3d_model(input_frame, matriz, coeffs, rvec, tvec, size);
                }
            }
