...
main (int argc, char* const* argv)
{
    ...
    try {
        ...
        cv::FileStorage fileS;
        fileS.open(calib_fname, cv::FileStorage::Mode::READ);
        fsiv_load_calibration_parameters(fileS, camera_size, error, K, dist_coeffs, rvec, tvec);
        //
        ...
        if (is_video)
        {
            ...
            cv::VideoCapture entrada(input_fname);
            double fps = entrada.get(cv::CAP_PROP_FPS);

            cv::VideoWriter salida(output_fname, cv::VideoWriter::fourcc(fourcc[0], fourcc[1], fourcc[2], fourcc[3]), fps, camera_size);
            fsiv_undistort_video_stream(entrada, salida, K, dist_coeffs, cv::INTER_LINEAR, "INPUT", "OUTPUT", fps);
            //
        }
        else
        {
            //TODO
            cv::Mat entrada = cv::imread(input_fname);
            cv::Mat salida = entrada.clone();
            
            fsiv_undistort_image(entrada, salida, K, dist_coeffs);

            cv::imshow("INPUT", entrada);
            cv::imshow("OUTPUT", salida);

            int key = cv::waitKey(0) & 0xff;
            if (key != 27)
            {
                if (!cv::imwrite(output_fname, salida))
                {
